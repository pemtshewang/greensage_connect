...
function Register() {
  ...
  const handleSubmitButton = async (data: SignUpSchemaType) => {
    setLoading(true);
    const res = await fetch("http://192.168.0.143:3000/api/user", {
      method: "POST",
      headers: {
  ...
  return (
    <OTPContext.Provider value={{ id, setID }}>
      <LinearGradient colors={["#228B29", "#6A9"]} style={LoginStyles.container}>
        ...
        <ScrollView>
          <VStack flex="1" space={1} alignItems="center" paddingX={10}>
            <View mb="3">
            ...
            <View style={RegisterStyles.linkView}>
              ...
              <Link href="/tabs/Home" style={RegisterStyles.link}>
                Move to home page
              </Link>
            </View>
          </VStack>
